...
The user may have noticed that in the example [above](#extracting), demonstrating `back`'s ability to mimic `basename`, is incomplete.  That is the outputs from `back` and `basename` are not identical.  This is because `basename`, by default, expands the tilde in the example `myfiles` whereas `back` does not.  Simply adding `expand_path` on the end of the chain replicates `basename` exactly. 
