...
**pathr** is a collection of tools to extract, examine, and reconfigure elements of filepaths.
...
|---------------------------|------------|-----------------------------------------------------------| 
| `pares_path`              | parsing    | Parse path into elements (sub-directories & files)        | 
...
| `normalize`               | manipulate | Make all path separators foward slashes                   | 
...
The `copy` function is also availavle and extends [`clipr::write_clipboard`](https://github.com/mdlincoln/clipr) to allow the user to easily copy a path to the clipboard but return the original value.  This makes `copy` pipeable.
